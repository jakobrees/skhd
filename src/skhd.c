...
static EVENT_TAP_CALLBACK(key_handler)
{
    ...
    case kCGEventFlagsChanged: {
        ...
        uint32_t flags = CGEventGetFlags(event);
        uint64_t now = macos_get_wall_clock();
        struct event_tap *tap = (struct event_tap *)reference;
        
        // Check for left cmd state change
        bool lcmd_pressed = (flags & Event_Mask_LCmd) == Event_Mask_LCmd;
        
        if (lcmd_pressed != tap->lcmd.is_pressed) {
            if (lcmd_pressed) {
                // Key pressed
                double since_last_press = tap->lcmd.last_press_time ? 
                    macos_get_milliseconds_elapsed(tap->lcmd.last_press_time, now) : 
                    1000.0;  // Large value for first press
                
                double since_last_release = tap->lcmd.last_release_time ?
                    macos_get_milliseconds_elapsed(tap->lcmd.last_release_time, now) :
                    1000.0;  // Large value if no release yet
                
                // Only count as second press if:
                // 1. Less than 300ms since last press
                // 2. We've seen a release in between
                // 3. Release wasn't too long ago (avoid counting very delayed second press)
                if (since_last_press <= 300.0 && 
                    tap->lcmd.last_release_time && 
                    since_last_release <= 300.0) {
                    
                    // Valid double-tap detected
                    struct hotkey eventkey = {
                        .flags = Hotkey_Flag_LCmdDouble,
                        .key = SKHD_MODIFIER_ONLY_EVENT
                    };
                    ...
                    // Reset state
                    tap->lcmd.last_press_time = 0;
                    tap->lcmd.last_release_time = 0;
                } else {
                    // First press in potential sequence
                    tap->lcmd.last_press_time = now;
                }
            } else {
                // Key released
                tap->lcmd.last_release_time = now;
            }
            
            tap->lcmd.is_pressed = lcmd_pressed;
        }
    } break;
    case NX_SYSDEFINED: {
        if (table_find(&blacklst, carbon.process_name)) return event;
...
int main(int argc, char **argv)
{
    ...
    event_tap.mask = (1 << kCGEventKeyDown) | (1 << kCGEventFlagsChanged) | (1 << NX_SYSDEFINED);
    // Initialize lcmd state
    event_tap.lcmd.is_pressed = false;
    event_tap.lcmd.last_press_time = 0;
    event_tap.lcmd.last_release_time = 0;
