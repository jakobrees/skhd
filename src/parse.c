...
parse_modifier(struct parser *parser)
{
    ...
    for (int i = 0; i < array_count(modifier_flags_str); ++i) {
        if (token_equals(modifier, modifier_flags_str[i])) {
            ...
            // Special case for lcmd_lcmd - it doesn't need a key
            if (flags & Hotkey_Flag_LCmdDouble) {
                return flags;
            }
...
parse_hotkey(struct parser *parser)
{
    ...
    if ((found_modifier = parser_match(parser, Token_Modifier))) {
        ...
        // Special case for lcmd_lcmd - it doesn't need a key or dash
        if (hotkey->flags & Hotkey_Flag_LCmdDouble) {
            hotkey->key = SKHD_MODIFIER_ONLY_EVENT;
            goto after_key;
        }
    }
